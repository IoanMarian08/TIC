...
app.post('/register', async (req, res) => {
  ...
  if (emailExist) {
    res.send('Utilizator deja existent. Adresa de mail exista deja in baza de date')
  }
  else {
...
app.post('/login', async (req, res) => {
  ...
  if (snapshot.empty) {
    let response = {};
    response.message = "Nu a fost gasita nici-o adresa de mail";
    res.json(response);
  } else {
    emailFound = true;
    snapshot.forEach(doc => {
      bcrypt.compare(data.password, doc.data().password).then(async function (result) {
        if (result) {
          ...
          let response = {};
          response.message = "Parola nu corespunde adresei de mail mentionate";
          res.json(response)
        }
      });
    });
...
app.post('/laptopuri', async (req, res) => {
  for (let i = 0; i < 10; i++) {
    ...
    const res = await db.collection('Laptopuri').add(
      obj);
  }
...
app.post('/telefoane', async (req, res) => {
  for (let i = 0; i < 10; i++) {
    ...
    const res = await db.collection('Telefoane').add(
      obj);
  }
...
app.post('/desktopuri', async (req, res) => {
  for (let i = 0; i < 10; i++) {
    ...
    const res = await db.collection('Desktopuri').add(
      obj);
  }
...
app.get('/laptopuri/culoare', async (req, res) => {
  ...
  try {
    ...
    const snapShot = await db.collection('Laptopuri').where("culoare", "==", culoare).get()
    snapShot.forEach(doc => {
      let laptop = {
        id: doc.id,
...
app.get('/telefoane/culoare', async (req, res) => {
  ...
  try {
    const culoare = req.query.culoare;
    const snapShot = await db.collection('Telefoane').where("culoare", "==", culoare).get()
    snapShot.forEach(doc => {
      let telefon = {
        id: doc.id,
        culoare: doc.data().culoare,
...
app.get('/desktopuri/culoare', async (req, res) => {
  ...
  try {
    const culoare = req.query.culoare;
    const snapShot = await db.collection('Desktopuri').where("culoare", "==", culoare).get()
    snapShot.forEach(doc => {
      let desktop = {
        id: doc.id,
        culoare: doc.data().culoare,
...
app.get('/laptopuri/pret', async (req, res) => {
  ...
  try {
    ...
    const snapShot = await db.collection('Laptopuri').where("pret", "==", pret).get()
    snapShot.forEach(doc => {
      let laptop = {
        id: doc.id,
        culoare: doc.data().culoare,
...
app.get('/telefoane/pret', async (req, res) => {
  ...
  try {
    const pret = req.query.pret;
    const snapShot = await db.collection('Telefoane').where("pret", "==", pret).get()
    snapShot.forEach(doc => {
      let telefon = {
        id: doc.id,
        culoare: doc.data().culoare,
...
app.get('/desktopuri/pret', async (req, res) => {
  ...
  try {
    const pret = req.query.pret;
    const snapShot = await db.collection('Desktopuri').where("pret", "==", pret).get()
    snapShot.forEach(doc => {
      let desktop = {
        id: doc.id,
        culoare: doc.data().culoare,
...
app.get('/laptopuri/nume', async (req, res) => {
  ...
  try {
    ...
    const snapShot = await db.collection('Laptopuri').where("nume", "==", nume).get()
    snapShot.forEach(doc => {
      let laptop = {
        id: doc.id,
        culoare: doc.data().culoare,
...
app.get('/telefoane/nume', async (req, res) => {
  ...
  try {
    const nume = req.query.nume;
    const snapShot = await db.collection('Telefoane').where("nume", "==", nume).get()
    snapShot.forEach(doc => {
      let telefon = {
        id: doc.id,
        culoare: doc.data().culoare,
...
app.get('/desktopuri/nume', async (req, res) => {
  ...
  try {
    const nume = req.query.nume;
    const snapShot = await db.collection('Desktopuri').where("nume", "==", nume).get()
    snapShot.forEach(doc => {
      let desktop = {
        id: doc.id,
        culoare: doc.data().culoare,
...
app.get('/telefon/:id', async (req, res) => {
  try {
    ...
    let telefon = await db.collection('Telefoane').doc(id).get()
    if (telefon.exists) {
      res.send(telefon.data())
    } else {
      res.status(404).send('N-avem sherifule')
    }
  } catch (error) {
...
app.get('/desktop/:id', async (req, res) => {
  try {
    let id = req.params.id;
    let desktop = await db.collection('Desktopuri').doc(id).get()
    if (desktop.exists) {
      res.send(desktop.data())
    } else {
      res.status(404).send('N-avem sherifule')
    }
  } catch (error) {
...
app.put('/laptop/:id', async (req, res) => {
  try {
    let id = req.params.id
    let data = req.body
    let snapshot = await db.collection('Laptopuri').doc(id).update(data)
    if (!snapshot.exist) {
...
app.delete('/laptop/:id', async (req, res) => {
  try {
    ...
    if (!snapshot.exist) {
      ...
      res.send('Nu s-a gasit produsul specificat de tine. Nu se poate sterge nimic.')
    }
